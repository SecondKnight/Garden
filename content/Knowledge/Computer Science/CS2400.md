...
Oftentimes, we write software to deal with certain issues or to facilitate some tasks. But, there's a process to it. After all  First, we **analyze**. We can't solve our problems if we don't even understand them to begin with. Once we have enough knowledge for us to work on, we devise plans to solve it. Ofcourse, we can't do everything in one go– so we'll continue to draft plans until we make one that works. It's an **algorithm** of sorts. 

Given that we are Comp. Sci Majors, it is our job to convert this algorithm into working code. There's a flow chart that I could add here, but I haven't made it. I'll probably add it in the future if it's needed. 
...
Keywords are words that have a special meaning in the syntax. This means that they can't be used in
